...
for ind in "${!CHKPT_REFS[@]}"; do
    ...
    epoch=$(echo "$CHKPT_REF" | grep -oP 'epoch=\K[0-9]+')
    NEW_CHKPT_NAME="${CHKPT_NAME}_epoch${epoch}"
    NEW_EXTRA_TAG="${EXTRA_TAG}/epoch${epoch}"
    echo "index=$epoch  value=$CHKPT_REF"
        ...
        if [[ "$DATASET" == 'tf' || "$DATASET" == 'promoter' ]]; then
            for fold in "fold1" "fold2" "fold3" "fold4" "fold5"; do
                for version in "ref_genome" "snp_genome"; do
                    ...
                    $PREFIX_CMD -o ../output_logs/${MODEL_NAME}_${NEW_CHKPT_NAME}/$DATASET_NAME/trian$EST_TIME.out \
                        ...
                        checkpoint_name=$NEW_CHKPT_NAME \
